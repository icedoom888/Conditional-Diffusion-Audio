...
from resamplers import PerceiverResampler
...
class ZEmbedder(nn.Module):
    def __init__(self, n_features: int = 192, out_dim: int = 1024, depth: int = 2, dim_head: int = 64, heads: int = 8):
        super().__init__()
        self.n_features = n_features
        self.resampler = PerceiverResampler(dim=n_features, depth=depth, dim_head=dim_head, heads=heads)
        self.to_time_dim = nn.Sequential(
    ...
    def forward(self, x: Tensor) -> Tensor:
        # transpose image channels and time
        x = rearrange(x, "... c t -> ... t c")
        x = self.resampler(x)
        x = x.mean(dim=-2)
        x = self.to_time_dim(x)
        return x
...
def TimeConditioningPlugin(
    ...
    def Net(modulation_features: Optional[int] = None, **kwargs) -> nn.Module:
        ...
        additional_embedder = ZEmbedder(n_features=192, out_dim=modulation_features, depth=num_layers, dim_head=64, heads=8)if add_additional_features else None
        ...
        def forward(
            ...
            **kwargs,
            ...
            # embedd the img features
            if exists(additional_embedder):
                features = additional_embedder(features)
        ...
        return Module([embedder, mlp, net, additional_embedder], forward)
